...
class makeGifObj(QObject):
    _progress = pyqtSignal(str)
    _done = pyqtSignal()
    def __init__(self):
        super(makeGifObj, self).__init__()

        self.BLOCK_TERMINATOR = b'\x00'
        self.EXTENSION = b'\x21'
        self.HEADER = b'GIF89a'
        self.TRAILER = b'\x3b'
        self.ZERO = b'\x00'
    
    def check_dataset_range(self, dataset):
    ...
    def check_dataset_shape(self, dataset):
    ...
    def check_dataset(self, dataset):
        ...
        if isinstance(dataset, numpy.ndarray) and not len(dataset.shape) == 4:
            self.check_dataset_shape(dataset)
            ...
            for i, d in enumerate(dataset):
                ...
                try:
                    self.check_dataset_shape(d)
                    self.check_dataset_range(d)
    ...
    def try_fix_dataset(self, dataset):
    ...
    def _get_logical_screen_descriptor(self, image, colors):
        height = len(image)
        width = len(image[0])
        ...
        size_of_global_color_table = self.get_color_table_size(len(colors))
        packed_bits = int(
    ...
    def _get_global_color_table(self, colors):
        """Return a color table sorted in descending order of count.
        """
        global_color_table = b''.join(c[0] for c in colors.most_common())
        full_table_size = 2**(1+int(self.get_color_table_size(len(colors)), 2))
        repeats = 3 * (full_table_size - len(colors))
        self.ZEROs = struct.pack('<{}x'.format(repeats))
        return global_color_table + self.ZEROs


    # ------------------------------- Graphics Control EXTENSION --- #
    def _get_graphics_control_EXTENSION(self, delay_time=0):
        control_label = b'\xf9'
        block_size = 4
        disposal_method = '001'
    ...
    def _get_application_EXTENSION(self, loop_times=0):
    ...
    # ============================================= Image Block ====== #
    # --------------------------------------- Image Descriptor --- #
    def _get_image_descriptor(self, image, left=0, top=0):
    ...
    def _get_sub_image(self, image, colors, delay_time=0):
        graphics_control_EXTENSION = (
            self._get_graphics_control_EXTENSION(delay_time=delay_time)
        )
        image_descriptor = self._get_image_descriptor(image)
        image_data = self._get_image_data(image, colors)
        return b''.join((
            graphics_control_EXTENSION,
            image_descriptor,
            image_data,
    ...
    def _make_gif(self, dataset):
        image = self.get_image(dataset)
        colors = self.get_colors(image)
        yield self._get_logical_screen_descriptor(image, colors)
        yield self._get_global_color_table(colors)
        yield self._get_sub_image(image, colors)


    def _make_animated_gif(self, datasets, delay_time=2):
        self._progress.emit('')
        images = [self.get_image(d) for d in datasets]
        self._progress.emit('')
        color_sets = (self.get_colors(image) for image in images)
        self._progress.emit('')
        colors = Counter()
        self._progress.emit('')
        ...
        if len(colors) > 256:
            ...
            raise RuntimeError(msg.format(len(colors)))
        self._progress.emit('')
        yield self._get_logical_screen_descriptor(images[0], colors)
        self._progress.emit('')
        yield self._get_global_color_table(colors)
        self._progress.emit('')
        yield self._get_application_EXTENSION()
        self._progress.emit('')
        for image in images:
            self._progress.emit('')
            yield self._get_sub_image(image, colors, delay_time=delay_time)


    def write_gif(self, dataset, filename, fps = 50):
        ...
        #     print('wrong')
        dataset = self.try_fix_dataset(dataset)
        # self.check_dataset(dataset)
        ...
        def encode(d):
            ...
            if four_d or not isinstance(dataset, numpy.ndarray):
                return self._make_animated_gif(d, delay_time=delay_time)    
            else:
                return self._make_gif(d)
